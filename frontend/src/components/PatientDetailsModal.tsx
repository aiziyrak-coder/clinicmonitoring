...
function CustomConfirm({ isOpen, title, message, onConfirm, onCancel }: { isOpen: boolean, title: string, message: string, onConfirm: () => void, onCancel: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-zinc-900/35 backdrop-blur-sm p-4" role="presentation" onClick={onCancel}>
      <div
        role="alertdialog"
        aria-labelledby="confirm-discharge-title"
        className="bg-white border border-zinc-200 rounded-xl w-full max-w-sm p-6 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center space-x-3 mb-4 text-red-600">
          <AlertTriangle className="w-6 h-6" aria-hidden />
          <h3 id="confirm-discharge-title" className="text-lg font-bold text-zinc-900">{title}</h3>
        </div>
...
class ErrorBoundary extends Component<PatientDetailsErrorBoundaryProps, PatientDetailsErrorBoundaryState> {
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-900/35 backdrop-blur-sm p-4">
          <div className="bg-white border border-red-300 rounded-xl w-full max-w-lg p-6 shadow-2xl">
            <h2 className="text-red-600 text-xl font-bold mb-4">Xatolik yuz berdi</h2>
            <p className="text-zinc-700 font-medium mb-4">Bemor ma'lumotlarini yuklashda xatolik yuz berdi.</p>
            <pre className="bg-red-50 border border-red-100 p-4 rounded text-red-800 text-xs overflow-auto max-h-40">
            ...
            <div className="mt-6 flex justify-end">
              <button 
                ...
                className="px-4 py-2 bg-zinc-200 text-zinc-900 font-semibold rounded hover:bg-zinc-300"
              >
                Sahifani yangilash
              </button>
            </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-white border border-zinc-200 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-zinc-200 sticky top-0 bg-white/95 backdrop-blur z-10 shadow-sm">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 id="patient-details-title" className="text-2xl font-bold text-zinc-900">{maskedName}</h2>
              <div className="flex items-center space-x-3 text-sm text-zinc-600 font-medium mt-1">
              ...
              {alarm.level !== 'none' && (
                <div className={`mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                  alarm.level === 'red' ? 'bg-red-100 text-red-800 border border-red-200' :
                  alarm.level === 'yellow' ? 'bg-amber-100 text-amber-900 border border-amber-200' :
                  alarm.level === 'purple' ? 'bg-purple-100 text-purple-900 border border-purple-200' :
          ...
          <div className="flex items-center space-x-3">
            <button type="button" onClick={handleDischarge} className="flex items-center px-4 py-2 bg-red-50 hover:bg-red-100 text-red-800 rounded-lg transition-colors text-sm font-semibold border border-red-200">
              <UserMinus className="w-4 h-4 mr-2" aria-hidden />
              Javob berish
            </button>
            <button type="button" onClick={handleExport} className="flex items-center px-4 py-2 bg-zinc-200 hover:bg-zinc-300 text-zinc-900 rounded-lg transition-colors text-sm font-semibold">
              <Download className="w-4 h-4 mr-2" aria-hidden />
              Eksport (CSV)
            </button>
            <button type="button" onClick={closeModal} className="p-2 text-zinc-600 hover:text-zinc-900 bg-zinc-100 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors" aria-label="Yopish">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        {/* Content */}
        <div className="flex border-b border-zinc-200 overflow-x-auto" role="tablist" aria-label="Bemor bo'limlari">
          <button 
            type="button"
            role="tab"
            aria-selected={activeTab === 'overview'}
            onClick={() => setActiveTab('overview')}
            className={`px-6 py-3 text-sm font-medium flex items-center whitespace-nowrap transition-colors ${activeTab === 'overview' ? 'text-emerald-800 border-b-2 border-emerald-600 bg-emerald-50' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}
          >
          ...
          <button 
            ...
            className={`px-6 py-3 text-sm font-medium flex items-center whitespace-nowrap transition-colors ${activeTab === 'limits' ? 'text-emerald-800 border-b-2 border-emerald-600 bg-emerald-50' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}
          >
          ...
          <button 
            ...
            className={`px-6 py-3 text-sm font-medium flex items-center whitespace-nowrap transition-colors ${activeTab === 'medications' ? 'text-emerald-800 border-b-2 border-emerald-600 bg-emerald-50' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}
          >
          ...
          <button 
            ...
            className={`px-6 py-3 text-sm font-medium flex items-center whitespace-nowrap transition-colors ${activeTab === 'labs' ? 'text-emerald-800 border-b-2 border-emerald-600 bg-emerald-50' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}
          >
          ...
          <button 
            ...
            className={`px-6 py-3 text-sm font-medium flex items-center whitespace-nowrap transition-colors ${activeTab === 'notes' ? 'text-emerald-800 border-b-2 border-emerald-600 bg-emerald-50' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}
          >
          ...
          {activeTab === 'overview' ? (
            <>
              {noHl7VitalsYet && (
                ...
                >
                  <p className="font-semibold text-cyan-900 mb-1">Vitallar hali kelmagan</p>
                  <p className="text-zinc-800 font-medium">
                    ...
                    bu yerda bo‘lmasa, serverda <span className="font-mono text-cyan-900 bg-cyan-100/80 px-1 rounded">journalctl -u clinicmonitoring-daphne</span> da
                    «HL7: vitallar ajratilmadi» / «vitallar qabul qilindi» qatorlarini tekshiring. Kerak bo‘lsa
                    backend <span className="font-mono text-cyan-900 bg-cyan-100/80 px-1 rounded">.env</span> da
                    <span className="font-mono text-cyan-900 bg-cyan-100/80 px-1 rounded">HL7_DEBUG=true</span> (barcha xom loglar) yoki{' '}
                    <span className="font-mono text-cyan-900 bg-cyan-100/80 px-1 rounded">HL7_LOG_RAW_PREVIEW=true</span> /{' '}
                    <span className="font-mono text-cyan-900 bg-cyan-100/80 px-1 rounded">HL7_LOG_RAW_TCP_RECV=true</span> (vaqtincha), Daphne qayta ishga tushiring — logda xom HL7 ko‘rinadi (maxfiylik!).
                  </p>
                </div>
              )}
              {/* Info Cards */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-zinc-100 p-4 rounded-xl border border-zinc-200">
                  ...
                  <p className="text-zinc-900 font-semibold">{patient.diagnosis}</p>
                </div>
                <div className="bg-zinc-100 p-4 rounded-xl border border-zinc-200">
                  <div className="flex items-center text-zinc-600 mb-2">
                ...
                <div className="bg-zinc-100 p-4 rounded-xl border border-zinc-200">
                  ...
                  <div className="flex items-center">
                    <div className="w-full bg-zinc-200 rounded-full h-2.5 mr-3">
                      <div className={`h-2.5 rounded-full ${(patient.deviceBattery || 0) > 20 ? 'bg-emerald-600' : 'bg-red-600'}`} style={{ width: `${patient.deviceBattery || 0}%` }}></div>
                    </div>
                    <span className="text-zinc-900 font-mono text-sm font-semibold">{Math.round(patient.deviceBattery || 0)}%</span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-zinc-50 p-6 rounded-xl border border-zinc-200">
              <h3 className="text-lg font-semibold text-zinc-900 mb-6 flex items-center">
                <Settings2 className="w-5 h-5 mr-2 text-emerald-600" aria-hidden />
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                      <div className="flex items-center text-emerald-800 font-semibold mb-4">
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex-1">
                          ...
                          <input type="number" value={localLimits.hr.low} onChange={(e) => handleLimitChange('hr', 'low', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Yuqori chegara</label>
                          <input type="number" value={localLimits.hr.high} onChange={(e) => handleLimitChange('hr', 'high', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200" />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* SpO2 Limits */}
                    <div className="bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                      <div className="flex items-center text-cyan-800 font-semibold mb-4">
                        <Activity className="w-4 h-4 mr-2" />
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Pastki chegara</label>
                          <input type="number" value={localLimits.spo2.low} onChange={(e) => handleLimitChange('spo2', 'low', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-200" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Yuqori chegara</label>
                          <input type="number" value={localLimits.spo2.high} onChange={(e) => handleLimitChange('spo2', 'high', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-200" />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* NIBP Sys Limits */}
                    <div className="bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                      <div className="flex items-center text-zinc-800 font-semibold mb-4">
                        <Activity className="w-4 h-4 mr-2" />
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Pastki chegara</label>
                          <input type="number" value={localLimits.nibpSys.low} onChange={(e) => handleLimitChange('nibpSys', 'low', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Yuqori chegara</label>
                          <input type="number" value={localLimits.nibpSys.high} onChange={(e) => handleLimitChange('nibpSys', 'high', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200" />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* NIBP Dia Limits */}
                    <div className="bg-zinc-50 p-4 rounded-lg border border-zinc-200">
                      <div className="flex items-center text-zinc-700 font-semibold mb-4">
                        <Activity className="w-4 h-4 mr-2" />
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Pastki chegara</label>
                          <input type="number" value={localLimits.nibpDia.low} onChange={(e) => handleLimitChange('nibpDia', 'low', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-zinc-500 mb-1">Yuqori chegara</label>
                          <input type="number" value={localLimits.nibpDia.high} onChange={(e) => handleLimitChange('nibpDia', 'high', e.target.value)} className="w-full bg-white border border-zinc-200 rounded px-3 py-2 text-zinc-900 font-medium focus:outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200" />
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-zinc-900 flex items-center">
                <Pill className="w-5 h-5 mr-2 text-emerald-700" />
                Dori-darmonlar
              </h3>
              {patient.medications && patient.medications.length > 0 ? (
                <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
                  <table className="w-full text-left text-sm text-zinc-700">
                    <thead className="bg-zinc-100 text-xs uppercase text-zinc-500 font-semibold">
                      <tr>
                    ...
                    <tbody className="divide-y divide-zinc-200">
                      {patient.medications.map(med => (
                        <tr key={med.id} className="hover:bg-zinc-50">
                          <td className="px-4 py-3 font-semibold text-zinc-900">{med.name}</td>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-zinc-900 flex items-center">
                <FlaskConical className="w-5 h-5 mr-2 text-emerald-700" />
                    ...
                    <tbody className="divide-y divide-zinc-200">
                      {patient.labs.map(lab => (
                        <tr key={lab.id} className="hover:bg-zinc-50">
                          <td className="px-4 py-3 font-semibold text-zinc-900">{lab.name}</td>
                          <td className={`px-4 py-3 font-bold ${lab.isAbnormal ? 'text-red-700' : 'text-emerald-700'}`}>
                            {lab.value} {lab.unit}
                          </td>
            ...
            <div className="space-y-6">
              ...
              <div className="flex space-x-2">
                <input 
                  ...
                  className="flex-1 bg-white border border-zinc-200 rounded-lg px-4 py-2 text-zinc-900 font-medium placeholder:text-zinc-400 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200"
                  ...
                  [...patient.notes].reverse().map(note => (
                    <div key={note.id} className="bg-zinc-50 border border-zinc-200 p-4 rounded-xl">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-semibold text-zinc-900">{note.author}</span>
                        <span className="text-xs text-zinc-500 font-medium">{note.time ? format(new Date(note.time), 'dd.MM.yyyy HH:mm') : ''}</span>
                      </div>
                      <p className="text-zinc-700 text-sm font-medium">{note.text}</p>
                    </div>
                  ))
                ) : (
